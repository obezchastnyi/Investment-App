...
public partial class InvestmentAppDbContext
{
        ...
        IEnumerable<UserRole> roles)
    {
        ...
        // Investor -> OPR [READER]
        var readerRoleId = roles.First(r => r.Code == UserRoles.Reader.ToString()).Id;
        users.AddRange(investors.Select(investor => new User
            {
                Id = investor.Id,
                UserName = $"{investor.SurName}-{investor.Name}",
                UserRoleId = readerRoleId,
                // passcode: opr_pass
        ...
        // Expert [WRITER]
        var writerRoleId = roles.First(r => r.Code == UserRoles.Writer.ToString()).Id;
        users.AddRange(experts.Select(expert => new User
        {
            Id = expert.Id,
            UserName = $"{expert.SurName}-{expert.Name}",
            UserRoleId = writerRoleId,
